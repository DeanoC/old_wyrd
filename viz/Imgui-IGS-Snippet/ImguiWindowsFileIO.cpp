        ...
        ImVec2 size )
{
    ...
    static bool directory_browsing = true;
    static int  recent_selected = 0;
#if defined(WIN32)
	static vector<std::string> drive_list = getWindowsDrives();
#endif
    ...
    if( CollapsingHeader("Browse Directories") )
    {
...
#if defined(WIN32)
		std::vector<const char*> tmpDriveList;
        toCStringVec(tmpDriveList, drive_list );
        ...
        Text( "  " ); SameLine();
        PushItemWidth( 40 );
        if( ListBox( "  ", &drive_selected, tmpDriveList.data(), tmpDriveList.size() ) )
        {
