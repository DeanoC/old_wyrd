...
TEST_CASE("float 2 half 2 float", "[math]")
{
	using namespace Math;
	// this just confirms my conversion from rygs negative shift (which is undefined)
	// to a constant, it produces a warning (the reason so off by default)
//	uint32_t const rndbias1 = ((15 - 127) << 23) + 0xfff;
//	REQUIRE(rndbias1 == 0xc8000fff);
	REQUIRE(0.0f == half2float(float2half(0.0f)));
	REQUIRE(-1.0f == half2float(float2half(-1.0f)));
	REQUIRE(1.0f == half2float(float2half(1.0f)));

	REQUIRE(std::isnan(half2float(float2half(std::numeric_limits<float>::quiet_NaN()))));
	REQUIRE(std::numeric_limits<float>::infinity() == half2float(float2half(std::numeric_limits<float>::infinity())));
