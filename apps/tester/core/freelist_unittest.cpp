...
TEST_CASE("MTFreeList<uint64_t, uint64_t>", "[core/freelist]")
{
	...
	{
		MTFreeList<uint64_t, uint64_t> freeList(3);
		REQUIRE(freeList.empty());
		REQUIRE(freeList.alloc() == 0);
		REQUIRE(freeList.alloc() == 1);
	...
	{
		// MT threaded tests
		constexpr int ThreadCount = 10000;
		MTFreeList<uint64_t, uint64_t> freeList(ThreadCount);
		std::thread threads[ThreadCount];

		// spawn n threads: push, pop etc. to try and break things
		for(int i = 0; i < ThreadCount; ++i)
		{
			threads[i] = std::thread(
					[&freeList](int thread_id)
					{
						if(thread_id & 0x1)
						{
