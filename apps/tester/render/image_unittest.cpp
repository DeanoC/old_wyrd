...
auto ImageTester(uint32_t w_, uint32_t h_, uint32_t d_, uint32_t s_, Render::GenericTextureFormat fmt_, bool doLog_) -> void
{
	...
	auto img = const_cast<GenericImageHandle*>(&imgHandle)->mutableAcquire<GenericImage>();
	if(doLog_)
	{
