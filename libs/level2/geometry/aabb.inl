...
INLINE AABB::AABB( Math::vec3 const & min, Math::vec3 const& max ) :
	m_MinExtent( min ),
	m_MaxExtent( max ) {
}
...
INLINE AABB AABB::fromCenterAndHalfLength( Math::vec3 const& center, Math::vec3 const& halfLength ) {
...
INLINE Math::vec3 const& AABB::getMaxExtent() const
{
	return m_MaxExtent;
}
...
INLINE Math::vec3 AABB::getBoxCenter() const
{
...
INLINE Math::vec3 AABB::getHalfLength() const
{
	return (m_MaxExtent - m_MinExtent) * 0.5f;
}
...
INLINE void AABB::setBoxCenter( Math::vec3 const& center )
{
	Math::vec3 const oldCenter = getBoxCenter();
...
/// \param	pnt	The pnt.
INLINE void AABB::expandBy( Math::vec3 const& pnt )
{
...
{
	Math::vec3 const minI = Math::max( m_MinExtent, aabb.m_MinExtent );
	Math::vec3 const maxI = Math::min( m_MaxExtent, aabb.m_MaxExtent );
...
/// \return	true if it succeeds, false if it fails.
INLINE bool AABB::intersects( Math::vec3 const& _point ) const
{
