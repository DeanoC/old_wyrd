...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
	...
	const float line_height = ImMax(ImMin(window->DC.CurrentLineSize.y, g.FontSize + g.Style.FramePadding.y * 2),
									g.FontSize);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node)
{
	...
	ImVec2 off = dock_node ? ImVec2((float) (int) (-g.Style.ItemInnerSpacing.x * 0.5f) + 0.5f, 0.0f) : ImVec2(0.0f,
																											  0.0f);
							 ...
							 float min_size2, float hover_extend, float hover_visibility_delay)
{
	...
	// Render
	const ImU32 col = GetColorU32(held ? ImGuiCol_SeparatorActive : (hovered &&
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	{
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y),
										frame_bb.Max,
	...
	if(g.ActiveId == id)
	{
		...
		if(g.ActiveIdSource == ImGuiInputSource_Mouse)
		{
			...
			} else
			{
				...
				clicked_t = (slider_usable_sz > 0.0f) ? ImClamp((mouse_abs_pos - slider_usable_pos_min) / slider_usable_sz,
																0.0f,
																1.0f) : 0.0f;
				if(!is_horizontal)
		...
		} else if(g.ActiveIdSource == ImGuiInputSource_Nav)
		{
			...
			{
				clicked_t = SliderCalcRatioFromValueT<TYPE, FLOATTYPE>(data_type,
																	   *v,
																	   v_min,
																	   v_max,
																	   power,
		...
		if(apply_edit_back_to_user_buffer)
		{
						 ...
						 ImGuiInputTextFlags_CallbackAlways)) != 0)
			{
				...
				if(event_flag)
				{
					...
					const int utf8_selection_end = callback_data.SelectionEnd = ImTextCountUtf8BytesFromStr(text,
																											text +
					...
					if(callback_data.BufDirty)
					{
						...
						if(callback_data.BufTextLen > backup_current_text_length && is_resizable)
							edit_state.TextW.resize(
									edit_state.TextW.Size + (callback_data.BufTextLen - backup_current_text_length));
						edit_state.CurLenW = ImTextStrFromUtf8(edit_state.TextW.Data,
	   ...
	   (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		for(int n = 0; n < components; n++)
		{
			...
			if(flags & ImGuiColorEditFlags_Float)
				value_changed = value_changed_as_float = value_changed | DragFloat(ids[n],
																				   &f[n],
																				   1.0f / 255.0f,
																				   0.0f,
																				   hdr ? 0.0f : 1.0f,
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	float sv_picker_size = ImMax(bars_width * 1,
								 CalcItemWidth() - (alpha_bar ? 2 : 1) * (bars_width +
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
	...
	const ImRect interact_bb = display_frame ? frame_bb : ImRect(frame_bb.Min.x,
																 frame_bb.Min.y,
				   ...
				   ImVec2 graph_size)
{
	...
	if(values_count > 0)
	{
		...
		const ImU32 col_hovered = GetColorU32((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLinesHovered
...
bool ImGui::BeginMainMenuBar()
{
	...
	SetNextWindowSize(ImVec2(g.Viewports[0]->Size.x,
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
	...
	if(width_excess > 0.0f && (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown))
	{
		...
		while(width_excess > 0.0f && tab_count_same_width < tab_bar->Tabs.Size)
		{
			...
			float width_to_remove_per_tab_max = (tab_count_same_width < tab_bar->Tabs.Size) ? (
					width_sort_buffer[0].Width - width_sort_buffer[tab_count_same_width].Width) : (
	...
	// Tab List Popup
	const bool tab_list_popup_button = (tab_bar->Flags & ImGuiTabBarFlags_DockNode) != 0 &&
									   (tab_bar->Flags & ImGuiTabBarFlags_NoTabListPopupButton) == 0;
	...
	const float scrolling_speed = (tab_bar->PrevFrameVisible + 1 < g.FrameCount) ? FLT_MAX : (g.IO.DeltaTime *
																							  g.FontSize * 70.0f);
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
	...
	bool want_clip_rect = !avail_bar_rect.Contains(ImRect(window->DC.CursorPos,
														  window->DC.CursorPos +
														  ImVec2(scrolling_buttons_width, 0.0f)));
	if(want_clip_rect)
					  ...
					  ImGuiWindow* docked_window)
{
	...
	if(pressed || ((flags & ImGuiTabItemFlags_SetSelected) &&
				   !tab_contents_visible)) // SetSelected can only be passed on explicit tab bar, so we don't need to set WantFocusTabId
	...
	if(held && !tab_appearing && IsMouseDragging())
	{
		...
		if(!g.DragDropActive &&
		   ((tab_bar->Flags & ImGuiTabBarFlags_Reorderable) || (flags & ImGuiTabItemFlags_DockedWindow)))
		{
				...
				//if (!g.IO.ConfigDockingWithShift || g.IO.KeyShift)
				{
					...
					float threshold_y = (threshold_base * 1.5f) +
										ImClamp((ImFabs(g.IO.MouseDragMaxDistanceAbs[0].x) - threshold_base * 2.0f) *
												0.20f, 0.0f, threshold_base * 4.0f);
...
void ImGui::SetTabItemClosed(const char* label)
{
	...
	bool is_within_manual_tab_bar =
			(g.CurrentTabBar.Size > 0) && !(g.CurrentTabBar.back()->Flags & ImGuiTabBarFlags_DockNode);
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
	...
	if(g.Style.TabBorderSize > 0.0f)
		draw_list->AddPolyline(draw_list->_Path.Data,
									   ...
									   const char* label, ImGuiID tab_id, ImGuiID close_button_id)
{
	...
	ImRect text_pixel_clip_bb(bb.Min.x + style.FramePadding.x,
	...
	if(flags & ImGuiTabItemFlags_UnsavedDocument)
	{
		...
		ImVec2 unsaved_marker_pos(ImMin(bb.Min.x + style.FramePadding.x + label_size.x + 2, text_pixel_clip_bb.Max.x),
	...
	if(label_size.x > text_ellipsis_clip_bb.GetWidth())
	{
		...
		float label_size_clipped_x = g.Font->CalcTextSizeA(g.FontSize,
														   text_ellipsis_clip_bb.GetWidth() - ellipsis_width + 1.0f,
														   0.0f,
														   label,
														   label_display_end,
														   &label_end).x;
		if(label_end == label && label_end <
								 label_display_end)    // Always display at least 1 character if there's no room for character + ellipsis
		{
		...
		{
			label_end--;
			label_size_clipped_x -= g.Font->CalcTextSizeA(g.FontSize,
														  FLT_MAX,
														  0.0f,
														  label_end,
														  label_end + 1).x; // Ascii blanks are always 1 byte
		}
