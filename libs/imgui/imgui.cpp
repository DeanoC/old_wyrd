...
#define
IMGUI_DEBUG_DOCKING_INI     0   // Save additional comments in .ini file (makes saving slower)
...
__GNUC__ >= 8
#pragma
GCC diagnostic ignored "-Wclass-memaccess"          // warning: 'memset/memcpy' clearing/writing an object of type 'xxxx' with no trivial copy-assignment; use assignment or value-initialization instead
#endif
#endif
...
#ifdef
IMGUI_ENABLE_TEST_ENGINE_HOOKS
...
{
#if
defined(_WIN32) && !defined(__CYGWIN__) && !defined(__GNUC__)
...
if(border_n == 1) {
border_posn = ImVec2(1, 0); border_target.x = (g.IO.MousePos.x - g.ActiveIdClickOffset.x + RESIZE_WINDOWS_FROM_EDGES_HALF_THICKNESS); }
if(border_n == 2) {
border_posn = ImVec2(0, 1); border_target.y = (g.IO.MousePos.y - g.ActiveIdClickOffset.y + RESIZE_WINDOWS_FROM_EDGES_HALF_THICKNESS); }
if(border_n == 3) {
border_posn = ImVec2(0, 0); border_target.x = (g.IO.MousePos.x - g.ActiveIdClickOffset.x + RESIZE_WINDOWS_FROM_EDGES_HALF_THICKNESS); }
...
if(g.NavWindow) {
for(int layer = 0; layer < 2; layer++) GetOverlayDrawList(g.NavWindow)->AddRect(g.NavWindow->Pos + g.NavWindow->NavRectRel[layer].Min, g.NavWindow->Pos + g.NavWindow->NavRectRel[layer].Max, IM_COL32(255,200, 0, 255)); } // [DEBUG]
if(g.NavWindow) {
ImU32 col = (g.NavWindow->HiddenFrames == 0) ? IM_COL32(255, 0, 255, 255) : IM_COL32(255, 0, 0, 255); ImVec2 p = NavCalcPreferredRefPos(NULL); char buf[32]; ImFormatString(buf, 32, "%d", g.NavLayer); GetOverlayDrawList(g.NavWindow)->AddCircleFilled(p, 3.0f, col); GetOverlayDrawList(g.NavWindow)->AddText(NULL, 13.0f, p + ImVec2(8, -4), col, buf); }
#endif
}
...
int depth = 0; while(node->ParentNode) { node = node->ParentNode; depth++; } return depth; }
...
line = ImStrSkipBlank(line + strlen("DockSpace")); node.IsDockSpace = true; }
else return;
...
line += r; node.Pos = ImVec2ih((short)x, (short)y); } else return;
...
line += r; node.IsCentralNode = (x != 0); }
if(sscanf(line, " SelectedTab=0x%08X%n", &node.SelectedTabID, &r) == 1) {
line += r; }
ImGuiDockContext* dc = ctx->DockContext;
if(node.ParentID != 0)
...
else if(sscanf(line, "DockFamilyId=0x%X", &u1) == 1)       {
settings->DockFamilyId = u1; }
}
...
#if
defined(_WIN32) && !defined(_WINDOWS_) && !defined(IMGUI_DISABLE_WIN32_FUNCTIONS) && (!defined(IMGUI_DISABLE_WIN32_DEFAULT_CLIPBOARD_FUNCTIONS) || !defined(IMGUI_DISABLE_WIN32_DEFAULT_IME_FUNCTIONS))
#ifndef
WIN32_LEAN_AND_MEAN
#define
WIN32_LEAN_AND_MEAN
