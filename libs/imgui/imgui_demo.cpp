...
void ImGui::ShowDemoWindow(bool* p_open)
{
	...
	ImGui::PushItemWidth(ImGui::GetFontSize() *
						 -12);           // Use fixed width for labels (by passing a negative value), the rest goes to widgets. We choose a width proportional to our font size.
	...
	if(ImGui::CollapsingHeader("Configuration"))
	{
		...
		if(ImGui::TreeNode("Configuration##2"))
		{
			...
			ImGui::SameLine();
			ShowHelpMarker(
					"Use SHIFT to dock window into another (or without SHIFT if io.ConfigDockingWithShift == false)");

			ImGui::CheckboxFlags("io.ConfigFlags: ViewportsEnable",
								 (unsigned int*) &io.ConfigFlags,
								 ImGuiConfigFlags_ViewportsEnable);
			ImGui::SameLine();
			ShowHelpMarker("Toggling this at runtime is normally unsupported (it will offset your windows).");
			...
			ImGui::CheckboxFlags("io.ConfigFlags: ViewportsDecoration",
								 (unsigned int*) &io.ConfigFlags,
								 ImGuiConfigFlags_ViewportsDecoration);
			ImGui::SameLine();
			ShowHelpMarker(
					"Toggling this at runtime is normally unsupported (most platform back-ends won't refresh the decoration right away).");
			ImGui::CheckboxFlags("io.ConfigFlags: ViewportsNoMerge",
								 (unsigned int*) &io.ConfigFlags,
								 ImGuiConfigFlags_ViewportsNoMerge);
			ImGui::SameLine();
			ShowHelpMarker("All floating windows will always create their own viewport and platform window.");

			ImGui::Checkbox("io.ConfigDockingWithShift", &io.ConfigDockingWithShift);
			ImGui::SameLine();
			ShowHelpMarker("Enable docking when holding Shift only (allows to drop in wider space, reduce visual noise)");
...
static void ShowDemoWindowWidgets()
{
	...
	if(ImGui::TreeNode("Selectables"))
	{
		...
		{
			static bool selected[16] = {true, false, false, false, false, true, false, false, false, false, true, false,
	...
	if(ImGui::TreeNode("Filtered Text Input"))
	{
		...
		struct TextFilters
		{
			...
			{
				if(data->EventChar < 256 && strchr("imgui",
												   (char) data->EventChar))
					return 0;
				return 1;
			}
	...
	if(ImGui::TreeNode("Color/Picker Widgets"))
	{
		...
		if(ImGui::Button("Default: Uint8 + HSV + Hue Bar"))
			ImGui::SetColorEditOptions(
					ImGuiColorEditFlags_Uint8 | ImGuiColorEditFlags_HSV | ImGuiColorEditFlags_PickerHueBar);
	...
	if(ImGui::TreeNode("Drag and Drop"))
	{
...

		{
			...
			static const char* names[9] = {"Bobby", "Beatrice", "Betty", "Brianna", "Barry", "Bernard", "Bibi",
										   "Blaine", "Bryn"};
			for(int n = 0; n < IM_ARRAYSIZE(names); n++)
			{
				...
				{
					ImGui::SetDragDropPayload("DND_DEMO_CELL",
											  &n,
	...
	if(ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)"))
	{
		...
		if(item_type == 2)
		{
			ret = ImGui::Checkbox("ITEM: CheckBox",
								  &b);
		}                            // Testing checkbox
		if(item_type == 3)
		{
			ret = ImGui::SliderFloat("ITEM: SliderFloat",
									 &col4f[0],
									 0.0f,
									 1.0f);
		}   // Testing basic item
			...
			if(font_details_opened)
			{
				...
				if(ImGui::TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size))
				{
					...
					for(int base = 0; base < 0x10000; base += 256)
					{
						...
						if(count > 0 && ImGui::TreeNode((void*) (intptr_t) base,
														"U+%04X..U+%04X (%d %s)",
														base,
														base + 255,
														count,
...
static void ShowExampleAppConstrainedResize(bool* p_open)
{
	struct CustomConstraints // Helper functions to demonstrate programmatic constraints
	{
		...
		{
			data->DesiredSize = ImVec2(IM_MAX(data->DesiredSize.x,
											  data->DesiredSize.y),
									   IM_MAX(data->DesiredSize.x,
											  data->DesiredSize.y));
		}
...
static void ShowExampleAppSimpleOverlay(bool* p_open)
{
	...
	if(corner != -1)
	{
		...
		ImVec2 window_pos = ImVec2((corner & 1) ? (viewport->Pos.x + viewport->Size.x - DISTANCE) : (viewport->Pos.x +
																									 DISTANCE),
								   (corner & 2) ? (viewport->Pos.y + viewport->Size.y - DISTANCE) : (viewport->Pos.y +
																									 DISTANCE));
