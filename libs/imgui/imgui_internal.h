...
static inline float ImFloorStd(
		float x) { return floorf(x); }   // we already uses our own ImFloor() { return (float)(int)v } internally so the standard one wrapper is named differently (it's used by stb_truetype)
...
{
	return ImVec2(v.x * cos_a - v.y * sin_a,
				  v.x * sin_a + v.y * cos_a);
}
...
struct ImGuiWindowSettings
{
	...
	ImGuiWindowSettings()
	{
		Name = NULL;
		ID = 0;
		Pos = Size = ViewportPos = ImVec2(0, 0);
		ViewportId = DockId = DockFamilyId = 0;
		DockOrder = -1;
		Collapsed = false;
	}
};
...
enum ImGuiViewportFlagsPrivate_
{
	...
	1 << 10   // Normal viewports are associated to a single window. The main viewport can host multiple windows.
};
...
struct ImGuiViewportP : public ImGuiViewport
{
	...
	{
		Idx = -1;
		LastFrameActive = LastFrameOverlayDrawList = LastFrontMostStampCount = -1;
		LastNameHash = 0;
		CreatedPlatformWindow = false;
		Alpha = LastAlpha = 1.0f;
		PlatformMonitor = INT_MIN;
		Window = NULL;
		OverlayDrawList = NULL;
		RendererLastSize = ImVec2(-1.0f, -1.0f);
	}
...
public:
	...
	{
		return (Flags & ImGuiWindowFlags_MenuBar) ? DC.MenuBarOffset.y + CalcFontSize() +
													GImGui->Style.FramePadding.y * 2.0f : 0.0f;
	}
...
enum ImGuiTabBarFlagsPrivate_
{
	...
	ImGuiTabBarFlags_SaveSettings = 1
			<< 23   // FIXME: Settings are handled by the docking system, this only request the tab bar to mark settings dirty when reordering tabs
};
...
enum ImGuiTabItemFlagsPrivate_
{
	...
	ImGuiTabItemFlags_Preview = 1
			<< 21   // [Docking] Display tab shape for docking preview (height is adjusted slightly to compensate for the yet missing tab bar)
};
...
IMGUI_API ImGuiDockNode* DockBuilderGetNode(
		ImGuiID node_id);                    // Warning: DO NOT HOLD ON ImGuiDockNode* pointer, will be invalided by any split/merge/remove operation.
...
IMGUI_API ImGuiID DockBuilderSplitNode(ImGuiID node_id, ImGuiDir split_dir, float size_ratio_for_node_at_dir,
									   ImGuiID* out_id_dir, ImGuiID* out_id_other);
