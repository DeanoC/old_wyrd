...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx)
{
	...
	if(!g_pVB || g_VertexBufferSize < draw_data->TotalVtxCount)
	{
		...
		if(g_pd3dDevice->CreateCommittedResource(&props,
												 D3D12_HEAP_FLAG_NONE,
												 &desc,
												 D3D12_RESOURCE_STATE_GENERIC_READ,
												 NULL,
												 IID_PPV_ARGS(&g_pVB)) < 0)
			return;
	...
	for(int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		for(int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
		{
			...
			{
				const D3D12_RECT r = {(LONG) (pcmd->ClipRect.x - pos.x), (LONG) (pcmd->ClipRect.y - pos.y),
...
bool ImGui_ImplDX12_CreateDeviceObjects()
{
	...
	// Create the root signature
	{
		...
		g_pd3dDevice->CreateRootSignature(0,
...
void ImGui_ImplDX12_InvalidateDeviceObjects()
{
	...
	{
		if(g_pFrameResources[i].IB)
		{
			g_pFrameResources[i].IB->Release();
			g_pFrameResources[i].IB = NULL;
		}
		if(g_pFrameResources[i].VB)
		{
			g_pFrameResources[i].VB->Release();
