...
static void sSendMouseCallback(uSynergyContext* context)
{
	...
	// Send callback
	context->m_mouseCallback(context->m_cookie,
							 context->m_mouseX,
							 context->m_mouseY,
							 context->m_mouseWheelX,
							 context->m_mouseWheelY,
...
static void sSendJoystickCallback(uSynergyContext* context, uint8_t joyNum)
{
	...
	context->m_joystickCallback(context->m_cookie,
								joyNum,
								context->m_joystickButtons[joyNum],
								sticks[0],
								sticks[1],
								sticks[2],
								sticks[3]);
...
static void sUpdateContext(uSynergyContext* context)
{
	...
	if(context->m_receiveFunc(context->m_cookie,
							  context->m_receiveBuffer + context->m_receiveOfs,
							  receive_size,
	...
	if(packlen > USYNERGY_RECEIVE_BUFFER_SIZE)
	{
		...
		char buffer[128];
		sprintf(buffer,
				"Oversized packet: '%c%c%c%c' (length %d)",
				context->m_receiveBuffer[4],
				context->m_receiveBuffer[5],
				context->m_receiveBuffer[6],
				context->m_receiveBuffer[7],
				packlen);
		sTrace(context, buffer);
