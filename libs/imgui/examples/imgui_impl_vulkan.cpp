...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer)
{
	...
	if(!fd->VertexBuffer || fd->VertexBufferSize < vertex_size)
		CreateOrResizeBuffer(fd->VertexBuffer,
							 fd->VertexBufferMemory,
							 fd->VertexBufferSize,
							 vertex_size,
...
void ImGui_ImplVulkan_InvalidateDeviceObjects()
{
	...
	for(int i = 0; i < IMGUI_VK_QUEUED_FRAMES; i++)
	{
		...
		if(fd->VertexBuffer)
		{
			vkDestroyBuffer(g_Device, fd->VertexBuffer, g_Allocator);
			fd->VertexBuffer = VK_NULL_HANDLE;
		}
		if(fd->VertexBufferMemory)
		{
			vkFreeMemory(g_Device, fd->VertexBufferMemory, g_Allocator);
			fd->VertexBufferMemory = VK_NULL_HANDLE;
		}
		if(fd->IndexBuffer)
		{
			vkDestroyBuffer(g_Device, fd->IndexBuffer, g_Allocator);
			fd->IndexBuffer = VK_NULL_HANDLE;
		}
		if(fd->IndexBufferMemory)
		{
			vkFreeMemory(g_Device, fd->IndexBufferMemory, g_Allocator);
			fd->IndexBufferMemory = VK_NULL_HANDLE;
		}
	}
...
void ImGui_ImplVulkanH_CreateWindowDataCommandBuffers(VkPhysicalDevice physical_device, VkDevice device,
													  uint32_t queue_family, ImGui_ImplVulkanH_WindowData* wd,
...
void ImGui_ImplVulkanH_CreateWindowDataSwapChainAndFramebuffer(VkPhysicalDevice physical_device, VkDevice device,
...
void ImGui_ImplVulkanH_DestroyWindowData(VkInstance instance, VkDevice device, ImGui_ImplVulkanH_WindowData* wd,
										 const VkAllocationCallbacks* allocator)
{
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
	...
	VkResult err = (VkResult) platform_io.Platform_CreateVkSurface(viewport,
																   (ImU64) g_Instance,
																   (const void*) g_Allocator,
																   (ImU64 * ) & wd->Surface);
	check_vk_result(err);
	...
	ImGui_ImplVulkanH_CreateWindowDataSwapChainAndFramebuffer(g_PhysicalDevice,
															  g_Device,
															  wd,
															  g_Allocator,
															  (int) viewport->Size.x,
