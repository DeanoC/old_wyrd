...
branch;
str += cont >>
merge;  // cont appears before its dominator
str += branch >> std::vector<Block>({
cont, merge
});
str +=
merge;
...
str += loop >>
selection;
str += selection >> std::vector<Block>({
loop, merge
});
str +=
merge;
...
bad;
str += bad >>
entry;  // Cannot target entry block
str +=
end;
str += "OpFunctionEnd\n";
...
str +=
bad >>
badvalue;  // Check branch to a function value (it's not a block!)
str +=
end;
str += "OpFunctionEnd\n";
...
str += entry >>
bad;
str += bad >> std::vector<Block>({
entry, exit
});  // cannot target entry block
str +=
exit;
str += "OpFunctionEnd\n";
...
bad;
str += bad >> std::vector<Block>({
def, block1, block2, block3, entry
});
str += def >>
merge;
str += block1 >>
...
exit;
str +=
unreachable >> std::vector<Block>({
unreachablechildt, unreachablechildf
});
str += unreachablechildt >>
merge;
str += unreachablechildf >>
merge;
str += merge >>
exit;
str +=
...
loop1;
str += loop1 >>
loop1_cont_break_block;
str += loop1_cont_break_block >> std::vector<Block>({
loop1_merge, loop2
});
str += loop2 >> std::vector<Block>({
loop2, loop2_merge
});
str += loop2_merge >>
loop1;
...
ss <<
i;
if_blocks.emplace_back("if" + ss.
str(), SpvOpBranchConditional
);
if(is_shader)
if_blocks[i].SetBody("OpSelectionMerge %if_merge" + ss.
str()
+ " None\n");
merge_blocks.emplace_back("if_merge" + ss.
str(), SpvOpBranch
);
}
std::string str = header(GetParam()) + std::string(types_consts()) +
...
i<N - 1; i++) {
str +=
if_blocks[i] >> std::vector<Block>({
if_blocks[i + 1], merge_blocks[i]
});
}
str += if_blocks.
back()
...
str += entry >>
loop1;
str += loop1 >> std::vector<Block>({
loop2, exit
});
str += loop2 >> std::vector<Block>({
loop2, loop2_merge
});
str += loop2_merge >> std::vector<Block>({
be_block, exit
});
str += be_block >>
loop1;
...
str += cheader >> std::vector<Block>({
exit, be_block
});
str +=
exit;  //  Branches out of a continue construct
str += be_block >>
loop;
str +=
merge;
...
str += entry >> std::vector<Block>({
def, case0, case1, case2
});
str += case1 >>
phi;
str +=
...
str += inner_head >> std::vector<Block>({
...
str += loop >>
if_head;
str += if_head >> std::vector<Block>({
if_true, if_merge
});
str += if_true >>
if_merge;
str += if_merge >> std::vector<Block>({
loop, merge
});
str +=
merge;
