...
ASSERT_EQ(
		SPV_SUCCESS,
		AssemblyContext(AutoText("Word"), nullptr)
.
...
ASSERT_EQ(
		SPV_SUCCESS,
		AssemblyContext(AutoText("Wo;rd"), nullptr)
.
getWord(& word, & endPosition
));
...
ASSERT_EQ(SPV_SUCCESS,
		AssemblyContext(&text, nullptr)
.
getWord(& word, & endPosition
));
ASSERT_EQ(0u, endPosition.line);
ASSERT_EQ(len, endPosition
.column);
ASSERT_EQ(len, endPosition
.index);
ASSERT_EQ(full_text
...
wordIndex < 4; ++wordIndex) {
ASSERT_EQ(SPV_SUCCESS, data
.
getWord(& word, & endPosition
));
...
position()
.index);
ASSERT_STREQ(words[wordIndex], word
.
c_str()
);
...
ASSERT_EQ(SPV_END_OF_STREAM, data
.
advance()
);
}
...
ASSERT_STREQ("word" BACKSLASH, word.
c_str()
);
}
...
ASSERT_STREQ("word" BACKSLASH BACKSLASH, word.
c_str()
);
}
