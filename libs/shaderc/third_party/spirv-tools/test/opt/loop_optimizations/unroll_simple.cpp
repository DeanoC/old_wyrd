...
SinglePassRunAndCheck<LoopUnroller>(text, output,
false);
}
...
LoopUnroller loop_unroller;
SetDisassembleOptions(SPV_BINARY_TO_TEXT_OPTION_NO_HEADER);
SinglePassRunAndCheck<PartialUnrollerTestPass<2>>(text, output,
false);
}
...
SinglePassRunAndCheck <PartialUnrollerTestPass<3>>(text, output,
false);
}
...
EXPECT_TRUE(loop
.
FindNumberOfIterations(induction, &* condition
->
ctail(),
		& iterations
));
...
EXPECT_EQ(iterations,
2u);
SinglePassRunAndCheck<LoopUnroller>(text, expected,
false);
}
...
EXPECT_EQ(iterations,
9u);
SinglePassRunAndCheck <PartialUnrollerTestPass<2>>(text, expected,
false);
}
...
EXPECT_TRUE(inner_loop
.
HasUnrollLoopControl()
);

EXPECT_EQ(outer_loop
.
GetBlocks()
.
size(),
...
LoopUnroller loop_unroller;
SetDisassembleOptions(SPV_BINARY_TO_TEXT_OPTION_NO_HEADER);
SinglePassRunAndCheck<LoopUnroller>(multiple_phi_shader, output,
false);
}
...
LoopUnroller loop_unroller;
SetDisassembleOptions(SPV_BINARY_TO_TEXT_OPTION_NO_HEADER);
SinglePassRunAndCheck<LoopUnroller>(condition_in_header, output,
false);
}
...
SetDisassembleOptions(SPV_BINARY_TO_TEXT_OPTION_NO_HEADER);
SinglePassRunAndCheck <PartialUnrollerTestPass<3>>(text, expected,
true);
