...
SinglePassRunAndCheck<LoopFissionPass>(source, expected,
true);
...
SinglePassRunAndCheck<LoopFissionPass>(
		source, source,
true,
[](const RegisterLiveness::RegionRegisterLiveness&) {
return false;
}
);
}
...
SetDisassembleOptions(SPV_BINARY_TO_TEXT_OPTION_NO_HEADER);
SinglePassRunAndCheck<LoopFissionPass>(source, source,
true);
}
...
// default).
SinglePassRunAndCheck<LoopFissionPass>(source, expected_multiple_passes,
true,
1);
}
...
EXPECT_EQ(pre_pass_descriptor
.
NumLoops(),
3u);
EXPECT_EQ(pre_pass_descriptor
.pre_begin()->NumImmediateChildren(), 2u);
...
EXPECT_EQ(post_pass_descriptor
.
NumLoops(),
5u);
EXPECT_EQ(post_pass_descriptor
.pre_begin()->NumImmediateChildren(), 4u);
}
...
2u);
EXPECT_EQ(pre_pass_descriptor
.pre_begin()->NumImmediateChildren(), 0u);
