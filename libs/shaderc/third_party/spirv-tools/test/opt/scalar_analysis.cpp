...
->
GetType(), SENode::RecurrentAddExpr
);

EXPECT_EQ(simplified
->GetChild(0)->
GetType(), SENode::Constant
);
EXPECT_EQ(simplified
->GetChild(0)->AsSEConstantNode()->FoldToSingleValue(),
1);
...
EXPECT_EQ(simplified
->GetChild(1)->AsSEConstantNode()->FoldToSingleValue(),
1);

EXPECT_EQ(simplified
->GetChild(0), simplified->GetChild(1));
}
...
EXPECT_NE(rec
->GetChild(0), rec->GetChild(1));

EXPECT_EQ(rec
->GetOffset()->GetType(), SENode::ValueUnknown);

EXPECT_EQ(rec
->GetCoefficient()->GetType(), SENode::Constant);
EXPECT_EQ(rec
->GetCoefficient()->AsSEConstantNode()->
FoldToSingleValue(),
1u);
}
...
EXPECT_EQ(simplified
->AsSEConstantNode()->FoldToSingleValue(), 33u);
}
...
EXPECT_EQ(load_count,
6);
EXPECT_EQ(store_count,
6);
...
EXPECT_EQ(simplified_node
->
GetType(), SENode::Constant
);
EXPECT_EQ(simplified_node
->AsSEConstantNode()->FoldToSingleValue(), 0u);
...
EXPECT_EQ(simplified_node
->AsSEConstantNode()->FoldToSingleValue(), 1u);
...
EXPECT_EQ(simplified_node
->AsSEConstantNode()->FoldToSingleValue(), -1);
...
EXPECT_EQ(difference_simplified
->
GetType(), SENode::ValueUnknown
);
...
EXPECT_EQ(difference_inverse
->GetChild(0), difference_simplified);
}
...
DumpDot(std::cout,
true);
EXPECT_NE(phi_node_1,
nullptr);
EXPECT_NE(phi_node_2,
nullptr);

EXPECT_EQ(phi_node_1
->
GetType(), SENode::RecurrentAddExpr
);
EXPECT_EQ(phi_node_2
->
GetType(), SENode::CanNotCompute
);

SENode* simplified_1 = analysis.SimplifyExpression(phi_node_1);
SENode* simplified_2 = analysis.SimplifyExpression(phi_node_2);

EXPECT_EQ(simplified_1
->
GetType(), SENode::RecurrentAddExpr
);
EXPECT_EQ(simplified_2
->
GetType(), SENode::CanNotCompute
);
}
