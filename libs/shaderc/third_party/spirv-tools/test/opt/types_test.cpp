...
size();
i++) {
EXPECT_EQ(signednesses[i], types[i]
->
IsSigned()
);
}
...
EXPECT_EQ(t
->IsUniqueType(false), expectation)
<< "expected '" << t->
str()
<< "' to be a "
...
EXPECT_EQ(t
->
HasSameDecorations(decorationless
.
get()
),
t->
