...
std::sort(TestInstruction::created_instructions_
.
begin(),
		TestInstruction::created_instructions_
.
end()
);
...
EXPECT_EQ(&* new_element, inserted_instructions
.
back()
);
}
...
EXPECT_TRUE(new_instructions
.
empty()
);
EXPECT_EQ(&* new_element, created_instructions
.
front()
);
