...
EXPECT_EQ(SPV_LITERAL_TYPE_INT_32, l
.type);
...
EXPECT_EQ(SPV_LITERAL_TYPE_UINT_32, l
.type);
EXPECT_EQ(0, l.value.i32);
...
EXPECT_EQ(SPV_LITERAL_TYPE_INT_64, l
.type);
...
EXPECT_EQ(SPV_LITERAL_TYPE_UINT_64, l
.type);
...
EXPECT_EQ(SPV_LITERAL_TYPE_FLOAT_32, l
.type);
...
EXPECT_EQ(SPV_LITERAL_TYPE_STRING, l
.type);
EXPECT_EQ(std::get<1>(GetParam()), l
.str);
}
...
ss << "Integer " <<
GetParam()
.text << " does not fit in a "
<<
GetParam()
.bitwidth << "-bit "
<< (
GetParam()
.is_signed ? "signed" : "unsigned") << " integer";
