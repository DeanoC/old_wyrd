...
EXPECT_NE(nullptr,
compiler1 = shaderc_compiler_initialize()
);
EXPECT_NE(nullptr,
compiler2 = shaderc_compiler_initialize()
);
EXPECT_NE(nullptr,
compiler3 = shaderc_compiler_initialize()
);
...
EXPECT_EQ(1u,
shaderc_result_get_num_errors(assembling
.
result()
));
EXPECT_EQ(0u,
shaderc_result_get_num_warnings(assembling
.
result()
));
}
...
EXPECT_EQ(2u,
shaderc_result_get_num_errors(comp
.
result()
));
// Expects the number of warnings to be zero.
EXPECT_EQ(0u,
shaderc_result_get_num_warnings(comp
.
result()
));
}
...
shaderc_compile_options_add_macro_definition(options_
.
get(),
"E", 1u, "main",
4u);
...
: kMinimalShaderDisassemblySubstrings) {
...
// #version.
shaderc_compile_options_set_forced_version_profile(options_
.
get(),
450,
...
.
get(),
4242 /*unknown version*/, shaderc_profile_core);
ASSERT_NE(nullptr, compiler_.
get_compiler_handle()
);
...
shaderc_compile_options_set_optimization_level(
		options_
.
get(), shaderc_optimization_level_zero
);
const std::string disassembly_text =
		CompilationOutput(kMinimalShader, shaderc_glsl_vertex_shader,
						  options_.get(), OutputType::SpirvAssemblyText);
...
shaderc_compile_options_set_optimization_level(
		options_
.
get(), shaderc_optimization_level_performance
);
const std::string disassembly_text =
...
shaderc_compile_options_set_optimization_level(
		options_
.
get(), shaderc_optimization_level_size
);
const std::string disassembly_text =
		CompilationOutput(kMinimalShader, shaderc_glsl_vertex_shader,
						  options_.get(), OutputType::SpirvAssemblyText);
...
shaderc_compile_options_set_include_callbacks(
		options
.
get(), TestIncluder::GetIncluderResponseWrapper,
...
EXPECT_EQ(0u,
shaderc_result_get_num_warnings(comp_line
.
result()
));
...
EXPECT_EQ(0u,
shaderc_result_get_num_warnings(comp_global
.
result()
));
}
...
shaderc_compile_options_set_target_env(options_
.
get(),
...
shaderc_compile_options_set_source_language(options_
.
get(),
...
shaderc_compile_options_set_limit(options_
.
get(),
...
shaderc_compile_options_set_auto_bind_uniforms(options_
.
get(),
true);
...
get(),
true);
shaderc_compile_options_set_binding_base(options_
.
get(),
...
shaderc_compile_options_set_binding_base_for_stage(
		options_
.
get(), shaderc_vertex_shader, shaderc_uniform_kind_texture,
100);
const std::string disassembly_text = CompilationOutput(
...
shaderc_compile_options_set_hlsl_offsets(options_
.
get(),
false);
const std::string disassembly_text =
...
shaderc_compile_options_set_hlsl_functionality1(options_
.
get(),
true);
const std::string disassembly_text =
