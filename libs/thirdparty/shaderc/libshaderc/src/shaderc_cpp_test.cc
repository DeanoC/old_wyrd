...
EXPECT_TRUE(compiler1
.
IsValid()
);
EXPECT_TRUE(compiler2
.
IsValid()
);
EXPECT_TRUE(compiler3
.
IsValid()
);
}
...
ASSERT_FALSE(compiler_
.
IsValid()
);
ASSERT_TRUE(compiler2
.
IsValid()
);
}
...
options_.
SetTargetEnvironment(shaderc_target_env_opengl,
0);
...
EXPECT_THAT(result
.
GetErrorMessage(), HasSubstr("wrongname")
);
}
...
options_.
SetOptimizationLevel(shaderc_optimization_level_zero);
...
EXPECT_EQ(0u, compilation_result.
GetNumWarnings()
);
}
...
options.
SetIncluder(std::unique_ptr<TestIncluder>(new TestIncluder(fs))
);
...
// profile.
options_.
SetTargetEnvironment(shaderc_target_env_opengl_compat,
0);
...
options_.
SetSourceLanguage(shaderc_source_language_hlsl);
...
std::string assembly;
spvtools::SpirvTools(SPV_ENV_UNIVERSAL_1_0)
.
...
options_.
SetLimit(shaderc_limit_min_program_texel_offset,
-99);
...
options_.
SetLimit(shaderc_limit_max_program_texel_offset,
10);
...
options.SetAutoBindUniforms(true);
options.
SetBindingBase(shaderc_uniform_kind_texture,
44);
const std::string disassembly_text = AssemblyOutput(
		kShaderWithUniformsWithoutBindings, shaderc_glsl_vertex_shader, options);
...
options.SetAutoBindUniforms(true);
options.
SetBindingBase(shaderc_uniform_kind_sampler,
44);
const std::string disassembly_text = AssemblyOutput(
		kShaderWithUniformsWithoutBindings, shaderc_glsl_vertex_shader, options);
EXPECT_THAT(disassembly_text, HasSubstr("OpDecorate %my_tex Binding 0")
...
options.SetAutoBindUniforms(true);
options.
SetBindingBase(shaderc_uniform_kind_image,
44);
const std::string disassembly_text = AssemblyOutput(
		kShaderWithUniformsWithoutBindings, shaderc_glsl_vertex_shader, options);
EXPECT_THAT(disassembly_text, HasSubstr("OpDecorate %my_tex Binding 0")
...
options.SetAutoBindUniforms(true);
options.
SetBindingBase(shaderc_uniform_kind_buffer,
44);
const std::string disassembly_text = AssemblyOutput(
		kShaderWithUniformsWithoutBindings, shaderc_glsl_vertex_shader, options);
EXPECT_THAT(disassembly_text, HasSubstr("OpDecorate %my_tex Binding 0")
...
SetSourceLanguage(shaderc_source_language_hlsl);
options.
SetHlslRegisterSetAndBindingForStage(shaderc_fragment_shader,
"t4",
"9", "16");
const std::string disassembly_text = AssemblyOutput(
...
SetSourceLanguage(shaderc_source_language_hlsl);
options.
SetHlslRegisterSetAndBindingForStage(shaderc_vertex_shader,
"t4", "9",
"16");
const std::string disassembly_text = AssemblyOutput(
		kHlslFragShaderWithRegisters, shaderc_glsl_fragment_shader, options);
