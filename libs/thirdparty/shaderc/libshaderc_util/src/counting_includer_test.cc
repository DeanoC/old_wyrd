...
std::thread t1(
		[&includer]()
		{ includer.includeLocal("name1", "me", 0); });
std::thread t2(
		[&includer]()
		{ includer.includeSystem("name2", "me", 1); });
std::thread t3(
		[&includer]()
		{ includer.includeLocal("name3", "me", 2); });
t1.
join();
t2.
join();
