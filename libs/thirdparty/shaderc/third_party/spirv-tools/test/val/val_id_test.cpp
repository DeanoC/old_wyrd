...
%11 = OpCompositeConstruct %2 %7 %7
      ...
      OpReturn
      OpFunctionEnd)";
spvValidatorOptionsSetRelaxStoreStruct(options_,
true);
CompileSuccessfully(spirv
.
c_str()
...
spvValidatorOptionsSetRelaxLogicalPointer(options_,
true);
CompileSuccessfully(spirv
.
c_str()
...
spirv << "%entry = " << instr << " %_ptr_Uniform_float %deep_var" <<
elem;
for(
int i = 0;
i<depth;
...
const std::string elem = AccessChainRequiresElemId(instr) ? " %int_0 " : "";
std::ostringstream spirv;
spirv << kGLSL450MemoryModel <<
kDeeplyNestedStructureSetup;
...
<< "%int_0 %int_3 %int_1 %int_2" <<
std::endl;
spirv << "%entry = " << instr << " %_ptr_Uniform_float %blockName_var "
<< elem << "%int_0 %int_3 %int_1 %int_2 %int_0" <<
std::endl;
spirv << R"(
OpReturn
OpFunctionEnd
...
spirv << kGLSL450MemoryModel << kDeeplyNestedStructureSetup <<
std::endl;
spirv << "%matrix = OpLoad %mat4x3 %my_matrix" <<
std::endl;
spirv << "%float_entry = OpCompositeExtract  %float %matrix 0 1" <<
std::endl;
