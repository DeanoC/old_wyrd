...
EXPECT_TRUE(t
.
Disassemble(binary, & output_text
));
...
EXPECT_TRUE(t
.
Assemble(MakeModuleHavingStruct(10), & binary
));
...
opts.
SetUniversalLimit(spv_validator_limit_max_struct_members,
9);
std::stringstream os;
...
EXPECT_TRUE(t
.
Disassemble(binary, & optimized_text
));
