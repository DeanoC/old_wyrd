...
EXPECT_TRUE (EnumSet<uint32_t>()
.
HasAnyOf (EnumSet<uint32_t>())
);
}
...
EXPECT_TRUE(EnumSet<uint32_t>(100)
.
HasAnyOf (EnumSet<uint32_t>())
);
}
...
EXPECT_FALSE(set
.
HasAnyOf(EnumSet<uint32_t>(20)
));
EXPECT_FALSE(set
.
HasAnyOf(EnumSet<uint32_t>(600)
));
EXPECT_TRUE(set
.
HasAnyOf(EnumSet<uint32_t>(5)
));
EXPECT_TRUE(set
.
HasAnyOf(EnumSet<uint32_t>(300)
));
EXPECT_TRUE(set
.
HasAnyOf(EnumSet<uint32_t>(0)
));
}
...
EXPECT_TRUE(s
.
Contains(SpvCapabilityMatrix)
);
EXPECT_FALSE(s
.
Contains(SpvCapabilityShader)
);
EXPECT_FALSE(s
.Contains(static_cast<SpvCapability>(1000)));
}
...
EXPECT_TRUE(s
.Contains(static_cast<SpvCapability>(63)));
EXPECT_FALSE(s
.Contains(static_cast<SpvCapability>(64)));
EXPECT_FALSE(s
.Contains(static_cast<SpvCapability>(1000)));
}
...
.Contains(static_cast<SpvCapability>(1000)));
EXPECT_TRUE(s
.Contains(static_cast<SpvCapability>(0x7fffffffu)));
}
...
EXPECT_TRUE(s
.
Contains(SpvCapabilityKernel)
);
EXPECT_TRUE(s
.Contains(static_cast<SpvCapability>(42)));
}
