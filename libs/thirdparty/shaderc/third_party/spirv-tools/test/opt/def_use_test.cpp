...
CheckUse(tc
.du, &manager, context->module()->IdBound());
}
...
context->
InvalidateAnalyses(IRContext::Analysis::kAnalysisDefUse);
(void)context->
get_def_use_mgr();
...
: tc.candidates) {
context->
ReplaceAllUsesWith(candidate
.first, candidate.second);
}

EXPECT_EQ(tc
.after,
DisassembleModule(context
->
module()
));
CheckDef(tc
.du, context->get_def_use_mgr()->id_to_defs());
CheckUse(tc
.du, context->
get_def_use_mgr(), context
->module()->IdBound());
}
...
CheckDef(def_uses, context
->get_def_use_mgr()->id_to_defs());
...
EXPECT_EQ(SpvOpSwitch, GetUseOpcodes(context, id)
.
back()
);
}
...
CheckDef(tc
.expected_define_use, manager.
id_to_defs()
);
CheckUse(tc
.expected_define_use, &manager, context->module()->IdBound());
...
CheckDef(expected, manager
.
id_to_defs()
);
CheckUse(expected, & manager, context
.module()->IdBound());
}
...
if(tc.indices_for_inst_to_kill.
count(inst
->
result_id()
)) {
...
CheckDef(tc
.expected_define_use, context->get_def_use_mgr()->id_to_defs());
CheckUse(tc
.expected_define_use, context->
get_def_use_mgr(),
		context
->module()->IdBound());
}
...
for(
const std::string& expected_anno_inst
: tc.annotations) {
