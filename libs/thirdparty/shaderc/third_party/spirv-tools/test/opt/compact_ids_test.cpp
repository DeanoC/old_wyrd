...
SinglePassRunAndCheck<NullPass>(before, after,
false, false);
}
...
SetAssembleOptions(SPV_TEXT_TO_BINARY_OPTION_PRESERVE_NUMERIC_IDS);
SetDisassembleOptions(SPV_BINARY_TO_TEXT_OPTION_NO_HEADER);
SinglePassRunAndCheck<CompactIdsPass>(before, after,
false, false);
}
...
tools.
Disassemble(binary, & disassembly, SPV_BINARY_TO_TEXT_OPTION_NO_HEADER
);
...
std::string disassembly;
tools.
Disassemble(binary, & disassembly, SPV_BINARY_TO_TEXT_OPTION_NONE
);
...
context->
BuildInvalidAnalyses(compact_id_pass
.
...
std::string disassembly;
tools.
Disassemble(binary, & disassembly,
