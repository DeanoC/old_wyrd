...
.
IsSupportedLoop(loops[0])
);
EXPECT_FALSE(analysis
.
GetDependence(context
->get_def_use_mgr()->GetDef(38),
...
.
GetLowerBound(loop)
->AsSEConstantNode()->FoldToSingleValue(),
-10);
EXPECT_EQ(
		analysis
.
...
EXPECT_EQ(analysis
.
GetFirstTripInductionNode(loop),
		analysis
.GetScalarEvolution()->CreateConstant(-10));

EXPECT_EQ(analysis
.
GetFinalTripInductionNode(
		loop, analysis
.GetScalarEvolution()->CreateConstant(1)),
...
GetFirstTripInductionNode(loop),
		analysis
.GetScalarEvolution()->CreateConstant(-5));
...
GetFirstTripInductionNode(loop),
		analysis
.GetScalarEvolution()->CreateConstant(0));
...
GetFirstTripInductionNode(loop),
		analysis
.GetScalarEvolution()->CreateConstant(15));
...
.IsWithinBounds(0, 0, 0));
EXPECT_TRUE(analysis
.IsWithinBounds(0, -1, 0));
EXPECT_TRUE(analysis
.IsWithinBounds(0, 0, 1));
EXPECT_TRUE(analysis
.IsWithinBounds(0, -1, 1));
EXPECT_TRUE(analysis
.IsWithinBounds(-2, -2, -2));
EXPECT_TRUE(analysis
.IsWithinBounds(-2, -3, 0));
EXPECT_TRUE(analysis
.IsWithinBounds(-2, 0, -3));
EXPECT_TRUE(analysis
.IsWithinBounds(2, 2, 2));
EXPECT_TRUE(analysis
.IsWithinBounds(2, 3, 0));
...
EXPECT_FALSE(analysis
.IsWithinBounds(2, 3, 3));
EXPECT_FALSE(analysis
.IsWithinBounds(0, 1, 5));
EXPECT_FALSE(analysis
.IsWithinBounds(0, -1, -4));
EXPECT_FALSE(analysis
.IsWithinBounds(-2, -4, -3));
}
...
EXPECT_TRUE(analysis
.
IsProvablyOutsideOfLoopBounds(
		loop, delta, store
->AsSERecurrentNode()->GetCoefficient()));
}
