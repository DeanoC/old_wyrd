...
EXPECT_EQ(manager
.GetType(100), manager.GetType(200));
}
...
ASSERT_EQ(7u, manager.
NumTypes()
);
// Make sure we get ids correct.
ASSERT_EQ("uint32", manager.GetType(5)->
str()
);
ASSERT_EQ("float32", manager.GetType(6)->
str()
);
...
ASSERT_EQ(10u, manager.
NumTypes()
);
// Make sure we get ids correct.
ASSERT_EQ("uint32", manager.GetType(8)->
str()
);
ASSERT_EQ("float32", manager.GetType(9)->
str()
);
...
ASSERT_EQ(5u, manager.
NumTypes()
);
// Make sure we get ids correct.
ASSERT_EQ("uint32", manager.GetType(3)->
str()
);
ASSERT_EQ("float32", manager.GetType(4)->
str()
);
...
case 1:
case 2:
EXPECT_EQ("void", t.second->
str()
);
break;
case 3:
EXPECT_EQ("bool", t.second->
str()
);
break;
case 4:
EXPECT_EQ("uint32", t.second->
str()
);
break;
case 5:
EXPECT_EQ("float64", t.second->
str()
);
break;
default:
...
ASSERT_EQ(context
->get_type_mgr()->GetType(1u), nullptr);
ASSERT_NE(context
->get_type_mgr()->GetType(2u), nullptr);
...
Struct st({&u32});
ASSERT_EQ(context
->get_type_mgr()->GetId(&st), 2u);
context->get_type_mgr()->RemoveId(2u);
ASSERT_EQ(context
->get_type_mgr()->GetType(2u), nullptr);
ASSERT_EQ(context
->get_type_mgr()->GetId(&st), 0u);
}
...
context->get_type_mgr()->
RemoveId(id);
EXPECT_EQ(nullptr, context->get_type_mgr()->
GetType(id)
);
}
...
st.AddMemberDecoration(0, {
{
35, 4
}
});
st.AddMemberDecoration(1, {
{
35, 4
}
});
