...
: encoding_table) {
codes.
push_back(BitsToStream(entry.second.first, entry.second.second)
);
}
...
size();
++j) {
ASSERT_FALSE(codes[i]
== codes[j].substr(0, codes[i].
length()
))
...
EXPECT_TRUE(huffman
.Encode("e", &bits, &num_bits));
EXPECT_EQ(2u, num_bits);
EXPECT_EQ("11",
BitsToStream(bits, num_bits
));

EXPECT_TRUE(huffman
.Encode("a", &bits, &num_bits));
...
EXPECT_TRUE(huffman
.Encode("x", &bits, &num_bits));
EXPECT_EQ(5u, num_bits);
EXPECT_EQ("00001",
BitsToStream(bits, num_bits
));

EXPECT_FALSE(huffman
.Encode("y", &bits, &num_bits));
}
...
ASSERT_TRUE(huffman
.
DecodeFromStream(read_bit, & decoded
));
EXPECT_EQ("l", decoded);
...
const std::map<uint32_t, uint32_t> hist = {{1, 10},
										   {2, 5},
										   {3, 15}};
...
const std::map<uint64_t, uint32_t> hist = {{1001, 10},
										   {1002, 5},
										   {1003, 15}};
