...
struct IBundle {
	...
	// chunk name, stage, major version, minor version, smart pointer to the data
	using ChunkCreateFunc = std::function<bool(std::string_view, int, uint16_t, uint16_t, std::shared_ptr<void>)>;

	// the pointer passed to destroy is the unsmart version of that passed to Create
	// it isn't nessecarily the same as passed for FreeFunc after all stages have been
	// destroyed, prefix if request will be before this pointer
	// stage, pointer
	using ChunkDestroyFunc = std::function<auto(int, void*)->void>;

	/// id, stage, extra mem, function to handle this type
	using ChunkHandler = std::tuple<uint32_t, int, uint32_t, ChunkCreateFunc, ChunkDestroyFunc>;
	...
	virtual std::string_view getDirectoryEntry(uint32_t const index_) = 0;
