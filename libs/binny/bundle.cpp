...
std::pair<Bundle::ErrorCode, uint64_t> Bundle::read(
				std::string_view name_,
        		std::vector<ChunkHandler> const& handlers_,
        		bool forcePrefix_ )
{
	// read header
	Header header;
	...
	for(int j = 0; j < handlers_.size(); ++j)
	{
		...
		int stage;
		uint32_t extramem;
		std::tie( type, stage, extramem, std::ignore,std::ignore) = handlers_.at( j );
		handlerMap[type].at( stage ) = j + 1; // 1 indexed so 0 can indicate no handler
	...
	for(size_t i = 0; i < header.chunkCount; i++)
	{
		...
		for(int j = MaxHandlerStages-1; j >= 0; --j)
		{
			...
			if(handlerIndex > 0)
			{
				ChunkDestroyFunc destroyer = std::get<4>(handlers_.at(handlerIndex - 1));
		...
		auto ptr = std::shared_ptr<void>((void *) basePtr,
				 [localFree,destroyers,dataPtr]( void *ptr )
				 {
					 for(auto const [stage, destroyer] : destroyers)
					 {
						destroyer(stage, dataPtr);
					 }
					 localFree( ptr );
				 } );
		...
		for(int j = 0; j < MaxHandlerStages; ++j)
		{
			...
			{
				int stage;
				ChunkCreateFunc handler;
				uint32_t extramem;
				std::tie( std::ignore, stage, extramem, handler, std::ignore ) = handlers_.at( handlerIndex - 1 );
				...
				if(handler != nullptr)
				{
					...
					handler( dir.getName(), cheader->majorVersion, cheader->minorVersion, stage, ptr );
				}
			}
		}
	}
