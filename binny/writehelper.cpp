...
void WriteHelper::default_type(std::string const& type) const
{
...
void WriteHelper::write_byte_array(std::vector<uint8_t> const& barray)
{
	...
	{
		for (size_t i = 0; i < barray.size() - 1; i++)
		{
			o << std::to_string(barray[i]);
			if ((i % 80) == 79)
			{
				o << std::endl;
			}
		...
		o << std::to_string(barray[barray.size() - 1]) << std::endl;
	}
